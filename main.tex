...
\title{Artigo final de sistemas distribuídos}
...
Excelente escolha. O Kerberos é o padrão-ouro para ensinar segurança em Sistemas Distribuídos porque ele resolve o problema da autenticação em rede aberta sem enviar senhas, usando uma "Entidade Confiável".

Abaixo preparei uma estrutura completa em LaTeX já configurada com os dados da sua equipe e seguindo o modelo da SBC.

Instruções Prévias:

Para esse código funcionar, você precisa do arquivo de estilo da SBC (sbc-template.sty) e a imagem do logo da SBC (opcional) na mesma pasta. Se estiverem usando o Overleaf, basta procurar o template "SBC Conferences" e substituir o código principal pelo código abaixo.

Eu inseri comentários no código (tudo que vem depois de %) explicando exatamente o que vocês devem escrever em cada seção.
...
% Pacotes obrigatórios do template SBC
\usepackage{sbc-template}
\usepackage{graphicx,url}
\usepackage[utf8]{inputenc}
\usepackage[brazil]{babel}
%\usepackage[latin1]{inputenc}  % Use este se tiver problemas com acentuação no Windows antigo

\sloppy

% Título e Autores (Baseado na Lista da Equipe 1)
\title{Análise do Protocolo Kerberos: Autenticação e Segurança em Sistemas Distribuídos}

\author{Luiz Antonio L. F. Leite\inst{1}, Luiz Sérgio S. Maciel Filho\inst{1}, \\ Max José L. Pantoja Junior\inst{1}, Wesley Pontes Barbosa\inst{1}}


\address{Faculdade de Computação -- Universidade Federal do Pará (UFPA)
  \email{ {luiz.leite, luiz.filho, max.junior, wesley.barbosa}@exemplo.ufpa.br }
}
